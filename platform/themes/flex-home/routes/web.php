...
Route::group(['namespace' => 'Theme\FlexHome\Http\Controllers', 'middleware' => ['web', 'core']], function () {
    Route::group(apply_filters(BASE_FILTER_GROUP_PUBLIC_ROUTE, []), function () {
        Route::get(SlugHelper::getPrefix(Project::class, 'projects') . '/' . SlugHelper::getPrefix(City::class, 'city') . '/{slug?}', 'FlexHomeController@getProjectsByCity')
            ->name('public.project-by-city');

        Route::get(SlugHelper::getPrefix(Property::class, 'properties') . '/' . SlugHelper::getPrefix(City::class, 'city') . '/{slug?}', 'FlexHomeController@getPropertiesByCity')
            ->name('public.properties-by-city');

        Route::get(SlugHelper::getPrefix(Account::class, 'agents'), 'FlexHomeController@getAgents')->name('public.agents');
        Route::get(SlugHelper::getPrefix(Account::class, 'agents') . '/{username}', 'FlexHomeController@getAgent')->name('public.agent');
