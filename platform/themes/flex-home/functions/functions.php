...
if (is_plugin_active('location')) {
    Event::listen(ImportedCityEvent::class, function (ImportedCityEvent $event) {
        $slug = Arr::get($event->row, 'slug') ?: Arr::get($event->row, 'name');
        $slug = create_city_slug($slug, $event->city);
    ...
    add_action([BASE_ACTION_AFTER_CREATE_CONTENT, BASE_ACTION_AFTER_UPDATE_CONTENT], function ($type, $request, $object) {
        if (is_plugin_active('location') && $type == CITY_MODULE_SCREEN_NAME) {
 ...
 * @return array
 */
function handle_min_max_input($calc, $minInput, $maxInput, $default): array
{
    $minLabel = '';
    $maxLabel = '';
    $dropLabel = $default;
