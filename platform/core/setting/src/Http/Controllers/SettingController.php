    ...
    public function previewEmailTemplate(Request $request, string $type, string $module, string $template)
    {
        ...
        if (! empty($inputData)) {
            foreach ($inputData as $key => $value) {
                $inputData[$key] = BaseHelper::clean(BaseHelper::stringify($value));
            }
        ...
        $content = BaseHelper::clean($content);

        $backUrl = route('setting.email.template.edit', ['type' => $type, 'name' => $module, 'template_file' => $template]);

        return view('core/setting::preview-email', compact('variables', 'content', 'inputData', 'backUrl'));
    }
}
